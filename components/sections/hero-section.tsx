...
function HeroSection({ slide, onSelectSlide, onMouseEnter, onMouseLeave }: HeroSectionProps) {
  ...
  return (
    <section id="top" className="hero relative isolate min-h-[780px] h-[min(98dvh,1000px)] overflow-hidden text-[#f9f3e8]" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      ...
      <div className="mx-auto flex h-full max-w-[1440px] flex-col justify-between px-5 pb-10 pt-32 sm:px-8 sm:pb-14 lg:px-12 lg:pb-16">
        <div className="max-w-[620px]">
          <p className="eyebrow text-[#e6b96c]" data-testid="text-hero-eyebrow">{current.eyebrow}</p>
        ...
        <div className="mt-auto flex flex-col gap-8 pt-12 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-4" role="group" aria-label="Hero slideshow controls">
              ...
              <span className="font-mono-display text-[10px] tracking-[.2em] text-[#f9f3e8]" data-testid="text-hero-count">0{slide + 1} / 0{content.heroSlides.length}</span>
            </div>
          </div>
        </div>
      </div>
